...
#' @name checkDerivAuto
#' @rdname checkDerivAuto
#' @export checkDerivAuto
#'
checkDerivAuto <- function(np, parSim, obj, ord = 1, trans = identity, ..., derFunNam = "derObj"){
  ...
  for(ii in 1:np){
    ...
    der[[ii]] <- checkDeriv(obj = wrapDerFun(obj[[derFunNam]], dropHessian = TRUE), param = pars[ii, ], ord = ord)
  }
