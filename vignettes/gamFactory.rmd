...
## Test of derivatives with positive stack effects
...
gr <- function(param) {
  ...
  le <- lEtaList$le
  convertDerivStackNonNegative(param, theta, X, Z, le, lee = 0)$lb
}
# hessian function
hes <- function(param) {
  ...
  le <- lEtaList$le
  lee <- lEtaList$lee
  convertDerivStackNonNegative(param, theta, X, Z, le, lee)$lbb
}
...
d3r <- function(r) {
  ...
  optimization <- nlm(function(x) {
    out <- - ll(x) + .5 * lam * t(x) %*% S %*% x
    attr(out, "gradient") <- - gr(x) + lam * S %*% x
    attr(out, "hessian") <- - hes(x) + lam * S
    out
...
D1H <- data.frame(EX = as.numeric(d1h(r)), 
                  FD = jacobian(function(x) as.numeric(hes(d3r(x)$betaHat)), r))
...
## Positive stack effects with SHASH family using mgcv
...
# Here you can decide between linear change or sinusoidal change of alphas
a1 <- 0.2 + 0.6 * z / n
a2 <- 1 - a1
a1 <- cos(z * 2 * pi / n) ^ 2 * 0.8 + 0.1
a2 <- sin(z * 2 * pi / n) ^ 2 * 0.8 + 0.1
...
k <- 30 # n. of basis functions
form <- lapply(1:K, function(x) formula("y ~ s(z, k = 30)"))
mod <- gam(form, family = stackFamilySHASH(X))

plot(mod)
# If you want to plot the actual effect over the estimated one
# lines(1:n, nu[, 1] - mod$coefficients[1], col = "red")
# lines(1:n, nu[, 2] - mod$coefficients[k + 1], col = "red")

betaHat <- matrix(mod$coefficients, ncol = K)
nuHat <- model.matrix(mod)[, 1:k] %*% betaHat
aHat <- exp(nuHat)
muHat <- rowSums(X * aHat)
plot(muHat, mu, cex = .1)
abline(0, 1, col = "red")
plot(mu - muHat, cex = .1)
```
