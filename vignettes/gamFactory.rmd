...
ret <- createSHwithStack2(beta, theta, X, Z, le, lee, leee, d1b = 0, deriv = 1)
...
# gradient
GR <- cbind(gr(param), 
            as.numeric(jacobian(func = l, x = as.vector(param))))
plot(GR)
abline(0, 1)
cbind(GR, apply(GR, 1, function(x) round(diff(x), 7)))

# hessian
HEX <- cbind(as.numeric(hes(param)), 
             as.numeric(jacobian(func = gr, x = as.vector(param))))
plot(HEX)
abline(0, 1)
